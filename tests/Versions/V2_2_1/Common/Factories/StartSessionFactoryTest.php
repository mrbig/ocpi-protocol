...
class StartSessionFactoryTest extends TestCase
{
    ...
    public static function assertStartSession(?stdClass $json, ?StartSession $command): void
    {
        if($json === null) {
            ...
            Assert::assertEquals($json->connector_id ?? null, $command->getConnectorId());
            Assert::assertEquals($json->authorization_reference ?? null, $command->getAuthorizationReference());
        }
    }
}