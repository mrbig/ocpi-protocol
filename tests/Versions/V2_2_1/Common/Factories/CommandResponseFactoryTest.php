...
class CommandResponseFactoryTest extends TestCase
{
    ...
    public static function assertCommandResponse(?stdClass $json, ?CommandResponse $response): void
    {
        if($json === null) {
            ...
            Assert::assertEquals($json->timeout ?? null, $response->getTimeout());
            Assert::assertCount(count($json->message ?? []), $response->getMessages());
            foreach ($response->getMessages() as $index => $message) {
