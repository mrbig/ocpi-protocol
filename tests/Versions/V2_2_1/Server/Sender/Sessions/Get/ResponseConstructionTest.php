...
use Chargemap\OCPI\Versions\V2_2_1\Server\Sender\Sessions\Get\SenderSessionGetResponse;
...
/**
 * @covers \Chargemap\OCPI\Versions\V2_2_1\Server\Sessions\SenderSessionGetResponse
 */
class ResponseConstructionTest extends TestCase
{
    public function testShouldReturnEmptyArrayWithoutTokens(): void
    {
        $response = new SenderSessionGetResponse(self::getRequest(), 0, 10);
        $responseInterface = $response->getResponseInterface();
        $this->assertSame([], json_decode($responseInterface->getBody()->getContents(), true)['data']);
    }

    private function getRequest(): SenderSessionGetRequest
    {
        return new SenderSessionGetRequest(
