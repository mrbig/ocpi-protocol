...
/**
 * @covers \Chargemap\OCPI\Versions\V2_2_1\Client\Credentials\Put\PutCredentialsRequest
 */
class PutCredentialsRequestTest extends TestCase
{
    public function getGetServerRequestInterfaceData(): iterable
    {
        foreach (scandir(__DIR__ . '/Payloads/PutCredentialsRequest/') as $filename) {
            if ($filename !== '.' && $filename !== '..') {
                yield $filename => [
                    'payload' => file_get_contents(__DIR__ . '/Payloads/PutCredentialsRequest/' . $filename),
                ];
            }
        }
    }
    ...
    public function testGetServerRequestInterface(string $payload): void
    {
        ...
        OcpiTestCase::coerce('V2_2_1/Common/credentials.schema.json', json_decode($payload));

        $credentials = $this->createMock(Credentials::class);
        $credentials->expects(TestCase::atLeastOnce())->method('jsonSerialize')->willReturn($json);

        $registerCredentialsRequest = new PutCredentialsRequest($credentials);
